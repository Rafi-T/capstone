...
years = sorted(rawDataRegression['year'].unique())
years.insert(0,-1)
...
for year in years:
    if (year == -1):
        univ = rawDataRegression[(rawDataRegression['year'] >= appliedCoefficientRange[0]) & (rawDataRegression['year'] <= appliedCoefficientRange[1])]
...
for year in forecastYears:
    ...
    forecastDF = univ.to_frame().reset_index()
